...
select b.bidder_id,bidder_name,
(count(bid_status)/(select count(bidder_id) from ipl_bidding_details where bidder_id=b.bidder_id))*100 as bids_won_percent
from ipl_bidding_details b 
left join ipl_bidder_details c 
on b.bidder_id=c.bidder_id
where bid_status='won'
group by b.bidder_id,bidder_name;

-- Q2. Display the number of matches conducted at each stadium with stadium name, city from the database.
select * from ipl_match_schedule;
select * from ipl_stadium;
select b.stadium_name,b.city,count(c.stadium_id) as total_matches
from ipl_stadium b 
left join ipl_match_schedule c
on b.stadium_id=c.stadium_id
group by  b.stadium_name,b.city;
...
select team_name,bid_team,count(bidder_id) as total_bids
from ipl_team b 
left join ipl_bidding_details c 
on b.team_id=c.bid_team
...
select team_name,sum(matches_played) as total_matches_played ,sum(matches_won) as total_matches_won,sum(matches_lost) as total_matches_lost
from ipl_team b 
join ipl_team_standings c 
...
from ipl_team_players b 
join ipl_team c
on b.team_id=c.team_id
join ipl_player d
on b.player_id=d.player_id
where player_role='Bowler' and c.team_name='Mumbai Indians';
